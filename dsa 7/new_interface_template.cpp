...
vector<string> findPath(list<Vertex*> adj, string start, string end) {
    unordered_map<string, bool> visited;
    vector<string> path;
...
public:
    ...
    std::vector<std::string> dfs(std::string source, std::string destination) {
       // function to return path from source to destination using dfs
        vector<string> ans = findPath(vertices, source, destination);
        return ans;
        }
...
public:
    ...
    void process_parcels(std::string parcels_file_name){
        ...
        ifstream file2(parcels_file_name);
        // int ttick, priority;
        string str;
        // int flag2 = 1;
        // cout << parcels_file_name << endl;
        if(file2.is_open()){
            while(getline(file2,str)){
                int flag = 0;
                // cout << str << endl;
                // cout << str[1] << endl;
                string id , source , destination , strtick , strprt;
                ...
                // int ttick = stoi(strtick);
                // int priority = stoi(strprt);
                stringstream ss;
                ss << strtick;
                int ttick = 0;
                ss >> ttick; 
                ...
                int priority = 0;
                ss2 >> priority; 
                // Parcel* temp = new Parcel(ttick, id, source, destination, priority);
                // parcels.insert(pair<string, Parcel*>(id, temp));
                // cout << "time -- " << ttick << " id -- " << id << " org -- " << source << " dest -- " << destination << " prt -- " << priority << endl;
...
// int main() {
    ...
    // PRC prc = PRC(2, 2);
    // prc.create_graph("graph.txt"); // give path
    // cout << prc.graph.vertices.size() << endl;
    // prc.process_parcels("bookings.txt"); // give path
