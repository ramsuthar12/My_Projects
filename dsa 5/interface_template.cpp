...
Exploration* checkjunction(string stop,Exploration* exp, Trip* prevtrip,AVLNode* root){
    ...
    // if (root->getStopName() == stop){
        ...
        if(root->getStops().size() > 1){
            ...
            for(int i = 0 ; i<root->getStops().size() ; i++){
                    if(prevtrip->getNode()->getLine()->getLineName() != root->getStops()[i]->getLine()->getLineName()){
            ...
            // if(root->getStopName() == "Kashmere Gate"){
            //     cout << exp->getTrips().size() << "--size of KG trips" << endl;
            // }
            return exp;
        }
        else{
...
Path* getPath(MetroStop* stop, Trip* tt,AVLTree* tree){
    ...
    MetroStop* backtravtemp = nullptr;
    // cout << backtrav->getFare() << "--fare" << endl;
    while (tt != nullptr){
                // cout << tt->getNode()->getStopName() << "--trip is "<<  endl;
                // cout << tt->getx() << "--tripx" << endl;
                // cout << backtrav->getStopName() << "--we are back travsering 1" << endl;
        finalpath->addStop(backtrav);
        fare = fare + backtrav->getFare();
        while (backtrav != tt->getNode() ){
            if (tt->getx() == 1){
                backtrav = backtrav->getNextStop();
                // cout << backtrav->getStopName() << "--we are back travsering 2" <<  endl;
                finalpath->addStop(backtrav);
                fare = fare + backtrav->getFare();
                if(backtrav == tt->getNode()){
                    backtravtemp = backtrav->getNextStop();
                    // finalpath->addStop(backtrav);
                }
            }
            else if(tt->getx() == 2){
                backtrav = backtrav->getPrevStop();
                // cout << backtrav->getStopName() << "--we are back travsering 3" <<  endl;
                finalpath->addStop(backtrav);
                fare = fare + backtrav->getFare();
                if(backtrav == tt->getNode()){
                    backtravtemp = backtrav->getPrevStop();
                    // finalpath->addStop(backtrav);
                }
            }
        }
        ...
        // cout << newline << "-- jump to this line" << endl;
        metrostops = tree->searchStop(backtravtemp->getStopName())->getStops();
        for(int i = 0 ; i<metrostops.size() ; i++){
            if(metrostops[i]->getLine()->getLineName() == newline){
                backtrav = metrostops[i];
        ...
        while(stop != nullptr ){
            if(travtrip->getx() == 1){
                ...
                if (stop->getStopName() == destination){
                    Path* finalpath = getPath(stop,travtrip,tree);
