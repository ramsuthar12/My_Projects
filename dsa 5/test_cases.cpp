...
void testPopulateTree() {
    ...
    AVLNode* node = tree->searchStop("Kashmere Gate");
    cout << "Here is kashmere gate junction" << endl;
    for(int i = 0 ; i<node->getStops().size() ; i++){
        cout << node->getStops()[i]->getLine()->getLineName() << endl;
    }
...
void testFindPath() {
    ...
    Path* path = pathFinder->findPath("Shastri Nagar", "Indraprastha");
    // cout << "hello" << endl;
    assert(path != nullptr);
